...
>
  ...
  <section class="capture-primary">
    ...
    <?php elseif ($capture['text'] || $capture['type'] === 'text'): ?><div class="prose <?=$isTrashed ? '' : 'capture-editable'?>" data-capture-field="text" data-markup data-placeholder="Add text" role="textbox" aria-label="Capture text" aria-multiline="true" <?=$isTrashed ? '' : 'contenteditable="true"'?>><?=nl2br(htmlspecialchars((string)$capture['text']))?></div>
    ...
    <?php if (in_array($capture['type'], ['audio','image'], true) && trim((string)$capture['extracted_text']) !== ''): ?>
      <details class="extracted-text-card">
        <summary><?=$capture['type'] === 'audio' ? 'Transcript' : 'Extracted text'?></summary>
        <div class="prose <?=$isTrashed ? '' : 'capture-editable'?>" data-capture-field="extracted_text" data-markup data-placeholder="Add extracted text" role="textbox" aria-label="<?=$capture['type'] === 'audio' ? 'Transcript' : 'Extracted text'?>" aria-multiline="true" <?=$isTrashed ? '' : 'contenteditable="true"'?>><?=nl2br(htmlspecialchars($capture['extracted_text']))?></div>
      </details>
    <?php endif; ?>
  </section>
  ...
  <?php if ($debugEnabled && $debugRequests): ?>
    <?php $debugRequestHeading = 'Related debug request'; ?>
    <?php $debugRequestCard = true; ?>
    <?php require dirname(__DIR__) . '/devices/_debug_requests.php'; ?>
  <?php endif; ?>
</article>
