...
<?php foreach ($captures as $capture): ?>
  <?php
    ...
    $assignedListIds = array_values(array_filter(explode(
        ',',
        (string) ($capture['assigned_list_ids'] ?? ''),
    )));
    ?>
  <article class="capture-item capture-item-<?=htmlspecialchars($capture['type'])?>">
    <a
    ...
    <footer class="capture-item-footer">
      ...
      <?php if ($captureShowActions): ?>
        <?php if ($captureStatus === 'trash'): ?>
          <form method="post" action="/captures/<?=urlencode($capture['id'])?>/restore">
