...
client_capture_id: optional-stable-id</code></pre><p>The device token belongs only in the <code>Authorization</code> header. <code>client_capture_id</code> is optional: a supplied value is preserved for idempotent retries; otherwise Catch generates a random value. Success returns the per-user catch number:</p><pre><code>{
  "result": "334"
}</code></pre><p>For an image or PDF, send one iOS Shortcuts <strong>File</strong> (<strong>Datei</strong>) value in the singular multipart field <code>attachment</code>. The Shortcut endpoint rejects multiple files. Repeated requests with the same explicit <code>client_capture_id</code> do not create duplicate captures.</p></section>

  <section id="inputs"><h2>Supported inputs</h2><div class="input-matrix"><div><strong>Unknown</strong><code>type=unknown</code><span>Share Sheet input that Catch inspects; accepts configured image and audio formats plus PDF</span></div><div><strong>Text</strong><code>type=text</code><span>Notes, dictation, and selected text</span></div><div><strong>URL</strong><code>type=url</code><span>Link, page title, and optional context</span></div><div><strong>Image</strong><code>type=image</code><span>Original image with optional OCR text</span></div><div><strong>Audio</strong><code>type=audio</code><span>Original recording with an optional transcript in extracted text</span></div><div><strong>File</strong><code>type=file</code><span>Documents and other permitted file types</span></div><div><strong>Mixed</strong><code>type=mixed</code><span>Multiple types of content in one capture</span></div></div><p>For <code>unknown</code>, Catch detects the final stored type. A lone URL in <code>text</code> becomes a URL capture and Catch attempts a safe page-title lookup. OCR and transcripts stay in <code>extracted_text</code> when an image or audio attachment is the source. Rejection messages include the detected MIME type and file extension.</p></section>
