...
final class Application
{
    ...
    public function run(): void
    {
        ...
        $currentUser=$auth->user();if($currentUser&&!$access->allowsUser($currentUser)){$auth->logout();$currentUser=null;}$path=(string)(parse_url($_SERVER['REQUEST_URI']??'/',PHP_URL_PATH)?:'/');$publicPaths=['/coming-soon','/login','/auth/start','/auth/callback','/logout','/health','/pair'];$isApi=$path==='/api'||str_starts_with($path,'/api/');if($access->isPrerelease()&&!$currentUser&&!$isApi&&!in_array($path,$publicPaths,true)){\Catch\Http\Response::redirect('/coming-soon');}
        ...
        $f3->route('POST /api/extension/pairing-requests',[$apiExtension,'startPairing']);$f3->route('POST /api/extension/pairing-requests/@request/exchange',[$apiExtension,'exchangePairing']);$f3->route('POST /api/extension/disconnect',[$apiExtension,'disconnect']);
