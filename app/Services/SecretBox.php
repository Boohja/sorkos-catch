...
final class SecretBox
{
    ...
    public function __construct(Config $config)
    {
        ...
        if(strlen($material)<32)throw new \RuntimeException('app.key must contain at least 32 characters.');
    ...
    public function decrypt(string $encoded): string
    {
        ...
        if($payload===false||strlen($payload)<=SODIUM_CRYPTO_SECRETBOX_NONCEBYTES)throw new \RuntimeException('Encrypted value is invalid.');
        $nonce=substr($payload,0,SODIUM_CRYPTO_SECRETBOX_NONCEBYTES);
        $plain=sodium_crypto_secretbox_open(substr($payload,SODIUM_CRYPTO_SECRETBOX_NONCEBYTES),$nonce,$this->key);
        if($plain===false)throw new \RuntimeException('Encrypted value could not be decrypted.');
        return $plain;
    }
}
