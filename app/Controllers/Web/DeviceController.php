...
final class DeviceController
{
    ...
    private function url(array $device): string{$slug=strtolower(trim(preg_replace('/[^a-z0-9]+/i','-',iconv('UTF-8','ASCII//TRANSLIT',$device['name'])?:$device['name']),'-'));return '/devices/'.$device['id'].'-'.($slug?:'geraet');}

    public function index(): void
    {
        $user=$this->user();$devices=$this->devices->all($user['id']);foreach($devices as &$device)$device['url']=$this->url($device);
        $this->view->render('devices/index',['title'=>'Geräte','user'=>$user,'devices'=>$devices,'csrf'=>$this->csrf->token()]);
    }

    public function new(): void{$this->view->render('devices/new',['title'=>'Gerät hinzufügen','user'=>$this->user(),'error'=>$_SESSION['device_error']??null,'csrf'=>$this->csrf->token()]);unset($_SESSION['device_error']);}
    ...
    public function create(): never
    {
        ...
        if($kind!=='mobile'||!in_array($platform,['ios','ipados'],true)||$name===''){$_SESSION['device_error']='Bitte wähle Mobil sowie iOS oder iPadOS und gib dem Gerät einen Namen.';Response::redirect('/devices/new');}
